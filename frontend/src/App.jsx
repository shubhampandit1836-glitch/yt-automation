...
function Overview({ overview, runs, videos, events, onForceRun }) {
  ...
  return <>
    <SectionHeading
      ...
      title={overview?.automation?.ready ? 'Your channel is on autopilot.' : 'Your channel is ready for setup.'}
      action={<button className="button primary" onClick={onForceRun}><span>＋</span> Force a brief</button>}
    />
    <div className="hero-banner">
      <div className="hero-copy"><span className="live-dot">● {overview?.automation?.ready ? 'AUTOMATION STATUS' : 'SETUP STATUS'}</span><h1>{overview?.automation?.ready ? <>Every day, your channel runs<br /><em>without you.</em></> : <>Your automatic channel is<br /><em>ready to connect.</em></>}</h1><p>{overview?.automation?.ready ? 'Topics are found, videos are written, edited, checked and queued automatically. You only need to look here when something needs attention.' : 'The full worker, scheduler, QA gate and professional editor are installed. Connect the required free-tier providers in Autopilot before publishing is enabled.'}</p></div>
      ...
      <div className="hero-foot"><span>Next automatic upload</span><b>{overview?.automation?.ready ? 'Tonight · 00:30 IST' : 'After go-live setup'}</b><span className="hero-separator" /><span>{overview?.automation?.mode === 'autopilot' ? 'Autopilot is on' : 'Preview mode'}</span><StatusPill value={overview?.automation?.mode === 'autopilot' ? 'autopilot on' : 'setup needed'} tone={overview?.automation?.mode === 'autopilot' ? 'good' : 'warn'} /></div>
    </div>
